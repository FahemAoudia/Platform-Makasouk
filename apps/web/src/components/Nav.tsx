    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-gold/15 bg-cream/85 backdrop-blur-xl transition-colors dark:border-white/10 dark:bg-zinc-950/90">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-6 py-5">
        <Link href="/" className="group flex min-w-0 items-center gap-3">
          <motion.span
            className="h-px w-10 bg-gold dark:bg-gold/80"
            layoutId="nav-accent"
            transition={{ type: "spring", stiffness: 380, damping: 32 }}
          />
          <div>
            <p className="font-display text-lg tracking-[0.35em] text-bark dark:text-[#F5E9DA]">
              MAKASOUK
            </p>
            <p className="text-[25px] uppercase tracking-[0.55em] text-bark/50 dark:text-cream/45">
              منصة مقاسك
            </p>
          </div>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2 text-xs uppercase tracking-[0.2em] sm:gap-3">
          <LocaleThemeToolbar />
          {user ? (
            <>
            ...
            <Link
              href="/auth/login"
              className="rounded-full bg-[#1F3A2E] px-4 py-2 text-[#FCF8F2] transition hover:bg-[#1F3A2E]/90 dark:bg-[#2a4d3e] sm:px-5"
            >
              {t("nav.connexion")}
            </Link>
          )}
      ...
      {user && mobileOpen && (
        <div className="border-t border-gold/15 bg-cream/95 px-6 py-4 dark:border-white/10 dark:bg-zinc-950/95 md:hidden">
